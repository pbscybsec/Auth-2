...
window.oncontextmenu = function () {
    ...
    return false; // This prevents the default context menu from showing
};

document.addEventListener('keydown', function (event) {
    if (event.ctrlKey && event.shiftKey && (event.key === 'I' || event.key === 'i')) {
        console.log("Ctrl+Shift+I has been disabled.");
        event.preventDefault();
    } else if (event.ctrlKey && event.shiftKey && (event.key === 'C' || event.key === 'c')) {
        console.log("Ctrl+Shift+C has been disabled.");
        event.preventDefault();
    } else if (event.key === 'F12') {
        console.log("F12 has been disabled.");
        event.preventDefault();
    }
});
